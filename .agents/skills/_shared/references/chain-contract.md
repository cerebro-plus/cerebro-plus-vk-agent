...
`vk_content_report.html` — обязательный финальный артефакт. Его SHA-256 должен совпадать с `validation_report.json`, а `report_handoff.json` должен содержать артефакт с ролью `html`. Без этого цепочка не считается завершённой.
