...
- Финальный отчёт всегда прикреплять к ответу как проверенный файл `vk_content_report.html`, в том числе при повторном использовании. Ссылка, путь к файлу, текстовое резюме и предпросмотр печати не заменяют вложение. Не считать отчёт завершённым, если HTML не прикреплён, отсутствует или его SHA-256 не совпадает с `validation_report.json`.
